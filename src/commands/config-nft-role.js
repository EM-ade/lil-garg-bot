...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await interaction.reply({ content: 'An error occurred while accessing the bot configuration. Please try again later.', flags: 64 });
            return;
        }
        ...
        switch (subcommand) {
            case 'set':
                ...
                if (nftCount < 0) {
                    await interaction.reply({ content: 'NFT count cannot be negative.', flags: 64 });
                    return;
                }
                ...
                try {
                    ...
                    await interaction.reply({ content: `Successfully ${updated ? 'updated' : 'added'} NFT role tier: **${role.name}** requires **${nftCount}** NFTs.`, flags: 64 });
                } catch (error) {
                    logger.error(`Error saving BotConfig after setting NFT role tier: ${error.message}`);
                    await interaction.reply({ content: 'An error occurred while saving the configuration. Please try again later.', flags: 64 });
                }
                break;
            ...
            case 'remove':
                ...
                if (config.nftVerification.roleTiers.length < initialLength) {
                    try {
                        ...
                        await interaction.reply({ content: `Successfully removed NFT role tier for **${roleToRemove.name}**.`, flags: 64 });
                    } catch (error) {
                        logger.error(`Error saving BotConfig after removing NFT role tier: ${error.message}`);
                        await interaction.reply({ content: 'An error occurred while saving the configuration. Please try again later.', flags: 64 });
                    }
                } else {
                    await interaction.reply({ content: `No NFT role tier found for **${roleToRemove.name}**.`, flags: 64 });
                }
                break;

            case 'list':
                if (config.nftVerification.roleTiers.length === 0) {
                    await interaction.reply({ content: 'No NFT role tiers are currently configured for this guild.', flags: 64 });
                    return;
                }
