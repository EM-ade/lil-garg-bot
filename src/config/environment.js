...
function validateEnvironment() {
  ...
  if (missing.length > 0) {
    console.error(
      `[${new Date().toISOString()}] Missing required environment variables: ${missing.join(
        ", "
      )}`
    );
    throw new Error(
  ...
  console.log(
    `[${new Date().toISOString()}] All required environment variables are present.`
  );
  console.log(
    `[${new Date().toISOString()}] Database URL: ${process.env.MONGO_URL}`
  );
  console.log(
    `[${new Date().toISOString()}] Database Name: ${process.env.DB_NAME}`
  );
}
